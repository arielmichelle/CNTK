...
def compute_proposals(img, num_proposals, cfg):
    ...
    if cfg is None: cfg = {}
    use_dlib = True                                                 if 'use_dlib_proposals' not in cfg else cfg['use_dlib_proposals']
    roi_ss_scale = 100                                              if 'roi_ss_scale' not in cfg else cfg['roi_ss_scale']
    roi_ss_sigma = 1.2                                              if 'roi_ss_sigma' not in cfg else cfg['roi_ss_sigma']
    roi_ss_min_size = 20                                            if 'roi_ss_min_size' not in cfg else cfg['roi_ss_min_size']
    ...
    try:
        if cfg.CNTK.DEBUG_OUTPUT:
            print("number of roi proposals (ss|total): {}|{}".format(num_rects, scaled_rects.shape[0]))
    except:
        pass
